(function () {
  ...
  function panelMarkup(context) {
    ...
    return `<style>${panelCss()}</style><div class="yalru-cli-header"><div><div class="yalru-cli-eyebrow">Agent Terminal</div><h3>${escapeHtml(agent.name)}</h3></div><div class="yalru-cli-chip">${escapeHtml(agent.key)} · ${escapeHtml(agent.adapter)}</div></div><div class="yalru-cli-meta"><span>${escapeHtml(roomLabel)}</span><span>${escapeHtml(agent.role || "No role set")}</span></div>${discordSurfacesMarkup(discordSurfaces)}<details class="yalru-adapter-auth" open><summary>Adapter auth</summary><div class="yalru-adapter-row"><span>Adapter</span><strong>${escapeHtml(agent.adapter)}</strong></div><div class="yalru-cli-actions"><button type="button" data-action="adapter-status">Check adapter auth</button><button type="button" data-action="adapter-reconnect">Run reconnect in terminal</button></div><pre class="yalru-adapter-output" data-adapter-output aria-live="polite">Not checked yet. Status checks never read or print secret values.</pre></details><div class="yalru-pty-toolbar"><a href="${terminalHref}" target="_blank" rel="noreferrer">Open full terminal</a></div><div class="yalru-pty-frame-wrap"><iframe class="yalru-pty-frame" src="${terminalHref}" title="${escapeHtml(agent.name)} real terminal"></iframe></div><details class="yalru-cli-fallback"><summary>Command runner fallback</summary><div class="yalru-terminal-toolbar"><button type="button" data-action="terminal-start">Start</button><button type="button" data-action="terminal-refresh">Refresh</button><button type="button" data-action="terminal-clear">Clear</button><button type="button" data-action="terminal-restart">Restart</button><button type="button" data-action="status">Status</button></div><pre class="yalru-terminal-output" data-terminal-output aria-live="polite">Starting terminal...</pre><form class="yalru-terminal-input-row" data-terminal-form><span aria-hidden="true">$</span><input id="yalru-agent-terminal-input" data-terminal-input autocomplete="off" spellcheck="false" placeholder="pwd" /><button type="submit" class="primary">Run</button></form></details><details class="yalru-agent-message"><summary>Agent message</summary><label class="yalru-cli-label" for="yalru-agent-cli-message">Message</label><textarea id="yalru-agent-cli-message" class="yalru-cli-textarea" rows="4" placeholder="Work Packet, review request, CLI instruction"></textarea><div class="yalru-cli-command" data-command>${escapeHtml(context.commands.attach)}</div><div class="yalru-cli-actions"><button type="button" data-action="attach">Attach</button><button type="button" data-action="wake" class="primary">Send + Wake</button><button type="button" data-action="copy">Copy CLI</button></div><pre class="yalru-cli-output" data-output aria-live="polite">Ready.</pre></details>`;
  }
